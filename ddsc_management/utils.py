...
def get_datatables_records_detail_support(request, querySet, allowedColitems=[], details_view_name=None, searchableColumns=[]):
    """
    Usage:
        querySet: query set to draw data from.
        allowedColitems: optional whitelist of field names allowed to be displayed.
    ...
    for row in rows:
        ...
        if 'details_url' in colitems and details_view_name is not None:
            ...
            row.insert(idx, reverse_with_params(details_view_name, {'pk': pk}))
        aaData.append(row)
